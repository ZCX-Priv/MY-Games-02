...
playState.create = function(){
	...
	resizeGame = function(scale, bounds){
		...
		if(window.famobi.getOrientation() == "portrait"){
			...
			if(projectInfo.tutorial){
				...
				gameInfo.tutorialLabel.rotation = 1.57;
				gameInfo.tutorialLabel.x = -350;
				gameInfo.tutorialLabel.y = -250;
			...
			if(projectInfo.tutorial){
				...
				gameInfo.tutorialLabel.rotation = 0;
				gameInfo.tutorialLabel.x = -250;
				gameInfo.tutorialLabel.y = -350;
			}
	...
	function initTutorial(){
		...
		if(projectInfo.tutorial == true){
				gameInfo.tutorialLabel = new Phaser.Sprite(game, -250, -350, 'tutorial');
				gameInfo.tutCanvas.addChild(gameInfo.tutorialLabel);
			if(game.device.touch){
	...
	function initGUI(){
		...
		gameInfo.aiWin = new Phaser.Sprite(game, -85, -81, 'aiWin');
	...
	function resume(){
		...
		window.famobi_analytics.trackEvent("EVENT_RESUME").then(function() {
	...
	function gameToPopUpMenu(){
		...
		window.famobi_analytics.trackEvent("EVENT_PAUSE").then(gameToPopUpMenu);
	}
